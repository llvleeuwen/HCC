...
devtools::install_github('immunogenomics/presto') # install for findallmarkers!
...
```{r load in datasets}
sample.names <- c(list.files("//store/isilon/archive/MDL lab/Groups/Group Kwekkeboom/Members/Luca van Leeuwen/Protocols/R Scripts for scRNAseq/scRNAseqHCC/Data")) # get sample names 
...
seuratobjects.merged$condition <- ifelse(grepl("T_C", seuratobjects.merged$orig.ident, ignore.case = TRUE), "T_C", "NT_P") # retrieve condition of samples

DimPlot(seuratobjects.merged, group.by = c("seurat_clusters"), label = TRUE) # plot clusters
